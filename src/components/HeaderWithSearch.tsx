...
import { NAVIGATION_SECTIONS, NAVIGATION_SECTIONS_MOBILE } from '../constants/navigation';
        ...
        <div className="flex justify-between items-center py-4">
          ...
          <nav className="hidden md:flex space-x-8 justify-end">
            {NAVIGATION_SECTIONS.map((section) => {
              // Si es el botón de Productos, renderizar el dropdown en su lugar
              if (section.key === 'productos-btn') {
                return (
                  <ProductsDropdown
                    key={section.key}
          ...
          <div className="md:hidden border-t shadow-lg bg-white max-h-[calc(100vh-80px)] overflow-y-auto">
            <nav className="px-4 py-4 space-y-2">
              {NAVIGATION_SECTIONS_MOBILE.map((section) => {
                // Si es el menú de Productos móvil, renderizar el menú expandible
                if (section.key === 'productos-mobile') {
                  return (
                    <div key={section.key}>
                      <button
