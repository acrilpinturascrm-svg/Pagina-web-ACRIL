...
const buildCauchoCandidates = (productName: string) => {
  const basePath = `/images/products/caucho/`;
...
const LineaCaucho = () => {
  ...
  const lineaCaucho = PRODUCT_LINES.find(line => line.name === 'Línea de Caucho Mate');
    ...
    <section className="py-16 bg-gray-50 min-h-screen">
      <div className="container mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="text-center p-4 rounded-lg" style={{ backgroundColor: ACRIL_COLORS.chickYellow50 }}>
              ...
              <p className="text-sm text-gray-600">Superficie suave y elegante</p>
            </div>
            <div className="text-center p-4 rounded-lg" style={{ backgroundColor: ACRIL_COLORS.bloodRed100 }}>
              <h3 className="font-semibold text-gray-900">Lavable</h3>
              <p className="text-sm text-gray-600">Fácil limpieza y mantenimiento</p>
            </div>
            <div className="text-center p-4 rounded-lg" style={{ backgroundColor: ACRIL_COLORS.chickYellow100 }}>
              <h3 className="font-semibold text-gray-900">Gran Cobertura</h3>
              <p className="text-sm text-gray-600">Excelente poder cubriente</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {lineaCaucho.products.map((product: Product) => {
            const candidates = buildCauchoCandidates(product.name);
            return (
        ...
        {/* Call to Action específico para caucho */}
        <div className="mt-16 rounded-xl p-8 text-center text-white" style={{ background: `linear-gradient(to right, ${ACRIL_COLORS.bloodRed800}, ${ACRIL_COLORS.chickYellow600})` }}>
          <h3 className="text-2xl font-bold mb-4">¿Buscas el color perfecto para tu hogar?</h3>
          <p className="mb-6 max-w-2xl mx-auto" style={{ color: ACRIL_COLORS.chickYellow50 }}>
            DECOCRIL Caucho ofrece 32 colores cuidadosamente seleccionados para crear ambientes 
            acogedores y elegantes en tus espacios interiores.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button 
              onClick={() => {
                const whatsappNumber = import.meta.env.VITE_WHATSAPP_NUMBER || '584244597443';
                const message = 'Hola, me interesa DECOCRIL Caucho para mi hogar. ¿Pueden asesorarme?';
