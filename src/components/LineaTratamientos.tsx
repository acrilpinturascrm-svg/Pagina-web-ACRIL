...
const buildTratamientosCandidates = (productName: string) => {
  const basePath = `/images/products/tratamientos/`;
...
const LineaTratamientos = () => {
  ...
  const lineaTratamientos = PRODUCT_LINES.find(line => line.name === 'Línea de Tratamientos');
      ...
      <section className="py-16 bg-gray-50 min-h-screen">
        <div className="container mx-auto px-4">
          ...
          <div className="mb-8">
            ...
            <div className="text-center">
              <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
                {lineaTratamientos.name}
              </h1>
              <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-6">
                {lineaTratamientos.description}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 mb-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Características de la Línea de Tratamientos</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="text-center p-4 rounded-lg" style={{ backgroundColor: ACRIL_COLORS.bloodRed50 }}>
                ...
                <p className="text-sm text-gray-600">Limpieza y acondicionamiento de superficies</p>
              </div>
              <div className="text-center p-4 rounded-lg" style={{ backgroundColor: ACRIL_COLORS.chickYellow50 }}>
                <h3 className="font-semibold text-gray-900">Protección</h3>
                <p className="text-sm text-gray-600">Sellado y protección preventiva</p>
              </div>
              <div className="text-center p-4 rounded-lg" style={{ backgroundColor: ACRIL_COLORS.bloodRed100 }}>
                ...
                <p className="text-sm text-gray-600">Productos para cuidado continuo</p>
              </div>
              <div className="text-center p-4 rounded-lg" style={{ backgroundColor: ACRIL_COLORS.chickYellow100 }}>
                <h3 className="font-semibold text-gray-900">Especializado</h3>
                <p className="text-sm text-gray-600">Soluciones para casos específicos</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {lineaTratamientos.products.map((product: Product) => {
              const candidates = buildTratamientosCandidates(product.name);
              return (
              <div key={product.name} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 border border-gray-200">
                ...
                <div className="p-4 text-center">
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      {expandedProduct === product.name ? 'Ocultar' : 'Ver Información Técnica'}
                    </button>
                    ...
                    {expandedProduct === product.name && (
                      <div className="mt-3 p-3 bg-gray-50 rounded text-left">
                        <div className="space-y-2">
                          <div>
                            <h5 className="font-medium text-gray-900 text-xs">Rendimiento:</h5>
                            <p className="text-xs text-gray-600">
                              {product.name === 'SELLADORES' ? 
                                '8-12 m² por litro, dependiendo de la porosidad' :
                                '10-14 m² por litro en superficies lisas'
                              }
                            </p>
                          </div>
                          <div>
                            <h5 className="font-medium text-gray-900 text-xs">Tiempo de secado:</h5>
                            <p className="text-xs text-gray-600">Al tacto: 30-45 min | Para repintar: 2-4 horas</p>
                          </div>
                          <div>
                            <h5 className="font-medium text-gray-900 text-xs">Dilución:</h5>
                            <p className="text-xs text-gray-600">
                              {product.name === 'SELLADORES' ? 
                                'Diluir con agua 10-20% para primera mano' :
                                'Usar sin diluir o diluir máximo 5% con solvente'
                              }
                            </p>
                          </div>
                        </div>
          ...
          {/* Call to Action específico para tratamientos */}
          <div className="mt-16 rounded-xl p-8 text-center text-white" style={{ background: `linear-gradient(to right, ${ACRIL_COLORS.bloodRed800}, ${ACRIL_COLORS.chickYellow600})` }}>
            <h3 className="text-2xl font-bold mb-4">¿Necesitas preparar tu superficie?</h3>
            <p className="mb-6 max-w-2xl mx-auto" style={{ color: ACRIL_COLORS.chickYellow50 }}>
              Los tratamientos de superficie son fundamentales para garantizar la adherencia y durabilidad 
              de cualquier sistema de pintura. Cada superficie requiere un tratamiento específico.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button 
                onClick={() => {
                  const whatsappNumber = import.meta.env.VITE_WHATSAPP_NUMBER || '584244597443';
                  const message = 'Hola, necesito asesoría sobre tratamientos de superficie para mi proyecto.';
...
export default LineaTratamientos;
